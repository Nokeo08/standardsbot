...
def validChptrNum(num):
    return 0 < num and num <= 33

def getWCF(fromChptr, fromPara, toChptr, toPara):
...
def parseWCFArgs(numGroups):
    ...
    for numGroup in numGroups:
        ...
        for i in args:
            i = "".join(i.split())
            if '-' not in i:
                if i.isdigit() and validChptrNum(int(i)):
                    result.append([int(i), 1, int(i), CHPTRMAX[int(i)]]) # (ex: 1)
                elif ":" in i:
                    split = i.split(":")
                    if len(split) != 2:
                        malformed = True
                ...
                if len(split) != 2:
                    ...
                    if ":" not in split[1] and split[1].isdigit() and validChptrNum(int(split[1])):
                        result.append([int(split[0]), 1, int(split[1]), CHPTRMAX[int(split[1])]]) # 4-5
                    elif ":" in split[1]:
                    ...
                    if len(leftSplit) != 2:
                        ...
                        if ":" not in split[1] and split[1].isdigit() and validChptrNum(int(split[1])):
                            result.append([int(leftSplit[0]), int(leftSplit[1]), int(split[1]), CHPTRMAX[int(split[1])]]) # (ex. 9:10-11)
                        elif ":" in split[1]:
                            rightSplit = split[1].split(":")
                            if len(rightSplit) != 2:
                                malformed = True
